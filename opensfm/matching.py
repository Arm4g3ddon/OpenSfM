...
def _match_descriptors_impl(
    ...
    if matcher_type == "WORDS":
        ...
        if _CUDA_AVAILABLE:
            ...
            if matches is not None:
                pass  # Use CUDA results
            else:
                # Fall through to CPU FLANN below
                matches = None
        else:
            matches = None
    ...
    elif matcher_type in ("BRUTEFORCE", "BRUTEFORCE_CUDA"):
        if symmetric_matching:
...
def match_brute_force_cuda_symmetric(
    fi: np.ndarray,
    fj: np.ndarray,
    config: Dict[str, Any],
) -> List[Tuple[int, int]]:
...
def match_brute_force(
    ...
    Uses CUDA when available and no mask is provided, otherwise falls back to CPU.
